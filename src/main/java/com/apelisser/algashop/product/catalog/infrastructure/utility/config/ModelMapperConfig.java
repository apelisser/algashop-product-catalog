package com.apelisser.algashop.product.catalog.infrastructure.utility.config;
...
import com.apelisser.algashop.product.catalog.infrastructure.utility.Slugfier;
...
public class ModelMapperConfig {
    ...
    private void configuration(ModelMapper modelMapper) {
        ...
        modelMapper.createTypeMap(Product.class, ProductDetailOutput.class)
            .addMappings(mapping -> mapping.using(fromStringToSlugConverter).map(Product::getName, ProductDetailOutput::setSlug));

        modelMapper.createTypeMap(Product.class, ProductSumaryOutput.class)
            .addMappings(mapping -> {
                mapping.using(fromStringToSlugConverter).map(Product::getName, ProductSumaryOutput::setSlug);
                mapping.using(fromStringToShortStringConverter).map(Product::getDescription, ProductSumaryOutput::setShortDescription);
